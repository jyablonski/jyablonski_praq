...
- Stateful operators: Flink’s memory use heavily depends on how much state you keep _between_ events. For example, if you do windowing, joins, aggregations, or any complex event processing that requires keeping data in memory (state), you’ll need enough memory to hold that state.
