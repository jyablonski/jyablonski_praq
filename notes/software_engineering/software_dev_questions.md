...
- From there, you can implement various different deployment strategies:
  ...
  1. Canary Deploy - route a small % of traffic to servers running the new version to monitor behavior before full rollout
  1. Blue Green - deploy the new version alongside the old one, then flip traffic once validated.
...
- Every deployment should be reversible
  ...
  1. In ArgoCD, revert to a previous git commit and sync
  1. In Helm, `helm rollback <release> <revision>`
...
- Control Place -> maanges the cluster's state, decides what should happen and when. It includes:
  ...
  1. Scheduler assigns newly scheduled pods to appropriate nodes based on resource availability, rules, and other constraints
  1. Controller Manager runs controllers that ensure the desired state (like replica sets, nodes etc) is maintained
  1. etcd which is a consistent highly available key valuew store to store the entire cluster config and state like a source of truth
  1. Cloud Controller Manager (like EKS) to manage provisioning cloud resources
- Worker Nodes are what actually run the apps. They include:
  ...
  1. Container Runtime runs the actual container (Docker, containerd etc)
  1. Kube Proxy manages networking rules and enables communication between pods and services across nodes
  1. Pods are the smallest deployable unit in K8s. Each pod contains 1 or more containers that share storage, network, and lifecylce.
- How It Works:
  1. You define the desired state via YAML Files or `kubectl`
  1. The API Server accepts the request and stores the spec in `etcd`
  1. The scheduler picks nodes to run the pods
  1. Kubelets on the nodes receive instructions and start containers using the container runtime
  1. Kube Proxy routes traffic between services and pods as needed
