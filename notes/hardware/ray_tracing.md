...
1. **Z-Buffering** – The GPU determines which triangles are visible (handling occlusion).
1. **Shading & Texturing** – Lighting is applied using **simplified models** (e.g., Phong shading, normal maps, baked lighting) instead of simulating real light.
1. **Screen-Space Effects** – Effects like reflections and ambient occlusion are done in **post-processing** rather than being physically simulated.
