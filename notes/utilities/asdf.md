...
1. Install asdf
1. Add startup script to your `~/.bashrc` (`echo ". $HOME/.asdf/asdf.sh" >> ~/.bashrc`)
1. Install the plugin for the programming language of choice (`asdf plugin add python`)
1. Install the version of the programming language you want (`asdf install python latest` or `asdf install python 3.9.14`)
   1. See all available versions with `asdf list all python`
1. Set your default version globally with `asdf global python latest` or locally in that specific direcotry with `asdf local python latest`
