...
3 Way Handshake: 1. SYN - Client wants to establish connection with a server, so it sends a SYN segment to start communication and defines the sequence number to start segments with 2. SYN + ACK - The server responds to the client, acknowledging the request and responding with the sequence number it will start the segments with. 3. ACK - Client acknowledges the response from the server and they both establish a reliable connection to be used to start actual data/packet transfer.
