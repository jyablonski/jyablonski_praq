...
1. Create product embeddings for each item using a pretrained or custom embedding model
1. Create user embeddings based on user behavior like purchase, view/browser history, categories of interest etc
1. Store both embeddings in a vector database to enable fast similarity search
1. For each user, retrieve the most similar product embeddings by calculating similarity scores against stored embeddings
1. Serve the top-k closest products as personalized recommendations.
...
1. Semantic Search - Find documents or items related to a relevant query
1. Recommendation Systems - represent users and items as embedding vectors, compute similarities, and recommend items whose embeddings are closest to the user's embedding
1. Image Search - represent images as vectors to compare, classify, or search images
...
1. Amazon - Product Search and Recommendations
