...
def schedule_scraper(month):
    global schedule_df
    url = "https://www.basketball-reference.com/leagues/NBA_2021_games-{}.html".format(
        month
    )
    html = urlopen(url)
    soup = BeautifulSoup(html)
...
def get_advanced_stats():
    ...
    logging.info(
        f"Advanced Stats Function Successful, retrieving updated data for 30 rows"
    )
    return df
...
player_stats.to_sql(
    con=connection, name="player_stats", index=False, if_exists="replace"
)
...
transactions.to_sql(
    con=connection, name="transactions", index=False, if_exists="replace"
)
schedule_df.to_sql(con=connection, name="schedule", index=False, if_exists="replace")
advanced_stats.to_sql(
    con=connection, name="advanced_stats", index=False, if_exists="replace"
)
