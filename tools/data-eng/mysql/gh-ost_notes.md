...
1. After starting the migration and creating the gh-ost table with the `dummy_ghost_column`, immediately run the throttle command to temporarily pause it
1. *OPTIONAL* Run Drop Primary Key, add new Primary Key Statement if needed
1. Run the Alter Table Statement to add partitioning to the gh-ost table
1. Run the Alter Table Statement to drop the `dummy_ghost_column`
1. Run the un-throttle command to begin the gh-ost migration again
1. Run the chunk size command to set the chunk-size to a higher number
